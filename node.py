#!/usr/bin/env python3
"""
MICROCORE (MCX) COMPLETE NODE v20.0 - FULL
...
STRIPE INTEGRATION: Real credit card payments
...
# Hardcoded bootnodes - only 1-2 IPs needed
BOOTSTRAP_NODES = [
...
BTC_PAYMENT_ADDRESS = os.environ.get("BTC_PAYMENT_ADDRESS", "your_btc_address")
ETH_PAYMENT_ADDRESS = os.environ.get("ETH_PAYMENT_ADDRESS", "your_eth_address")
USDC_PAYMENT_ADDRESS = os.environ.get("USDC_PAYMENT_ADDRESS", "your_usdc_address")
DUCO_PAYMENT_ADDRESS = "XAVER_KENG_XUAN_YI"  # Fixed DUCO address
...
class DEX:
    ...
    def get_own_pool_quote(self, from_token, to_token, amount):
        if from_token == "MCX":
            ...
            fee = self._calc_fee_mcx(amount * self.mcx_price_usd)
            return {"out": out, "fee": fee, "type": "own", "pool": f"MCX/{to_token}"}
        else:
            ...
            fee = self._calc_fee_mcx(amount * self.mcx_price_usd)
            return {"out": out, "fee": fee, "type": "own", "pool": f"MCX/{from_token}"}
    ...
    def execute_own_pool_swap(self, wallet, from_token, to_token, amount, fee):
        ...
        pool = self.own_pools[quote["pool"]]
        if from_token == "MCX":
            pool["a"] += amount
            pool["b"] -= quote["out"]
        else:
    ...
    async def get_lifi_quote(self, from_token, to_token, amount):
        try:
            ...
            return {"out": out, "fee": fee, "type": "lifi", "route": f"{from_token} → {to_token} via LI.FI"}
        except:
    ...
    async def get_thorchain_quote(self, from_token, to_token, amount):
        try:
            ...
            fee = self._calc_fee_mcx(value_usd)
            return {"out": out, "fee": fee, "type": "thorchain", "route": f"{from_token} → {to_token} via THORChain"}
        except:
    ...
    async def execute_swap(self, wallet, from_token, to_token, amount, fee):
        if from_token == "MCX" or to_token == "MCX":
            return self.execute_own_pool_swap(wallet, from_token, to_token, amount, fee)
...
class LevelManager:
    ...
    def get_permanent_tower_amount(self, wallet):
        return self.perm_towers.get(wallet, 0)
...
def verify_btc_payment(address, expected_amount, confirmations=1):
    """Verify BTC payment using blockchain.info API"""
    try:
        response = requests.get(f"{BLOCKCHAIN_INFO_API}{address}")
        if response.status_code == 200:
            data = response.json()
            total_received = data.get('total_received', 0) / 1e8
            if total_received >= expected_amount:
                tx_count = len(data.get('txs', []))
                if tx_count >= confirmations:
                    return {"success": True, "received": total_received}
        return {"success": False, "error": "Payment not verified"}
    except Exception as e:
        return {"success": False, "error": str(e)}

def verify_eth_payment(address, expected_amount, confirmations=1):
    """Verify ETH payment using Etherscan API"""
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('status') == '1':
                ...
                if balance >= expected_amount:
                    return {"success": True, "received": balance}
        return {"success": False, "error": "Payment not verified"}
    except Exception as e:
        return {"success": False, "error": str(e)}

def verify_usdc_payment(address, expected_amount, confirmations=1):
    """Verify USDC payment using Etherscan API"""
    try:
        response = requests.get(
...
def verify_duco_payment(expected_amount):
    """Verify DUCO payment to XAVER_KENG_XUAN_YI"""
    try:
        response = requests.get(f"{DUCO_API}?address={DUCO_PAYMENT_ADDRESS}")
        if response.status_code == 200:
            data = response.json()
            if data.get('result'):
                balance = float(data['result'].get('balance', 0))
                if balance >= expected_amount:
                    return {"success": True, "received": balance}
        return {"success": False, "error": "DUCO payment not verified"}
    except Exception as e:
        return {"success": False, "error": str(e)}

def verify_crypto_payment(method, address, amount):
    """Unified crypto payment verification"""
    if method == "btc":
        return verify_btc_payment(address, amount)
    elif method == "eth":
        return verify_eth_payment(address, amount)
    elif method == "usdc":
        return verify_usdc_payment(address, amount)
    elif method == "duco":
        return verify_duco_payment(amount)
    else:
...
class MicroCoreNetwork:
    ...
    def _init_db(self):
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS pending_crypto_payments (
            ...
            method TEXT, amount INT, usd_amount REAL, address TEXT,
            status TEXT, created_at REAL, completed_at REAL)''')
        
        c.execute('''CREATE TABLE IF NOT EXISTS stripe_payments (
            payment_id TEXT PRIMARY KEY, wallet TEXT, username TEXT,
            usd_amount REAL, mcx_amount INT, status TEXT,
            created_at REAL, completed_at REAL, stripe_payment_intent TEXT)''')
    ...
    def _register_self_miner(self):
        ...
        self.conn.commit()
        print(f"[EMBEDDED] Miner '{self.username}' active (stake: 1000 MCX, Level: 1)")
...
# ==================== PAYMENT PROCESSING ====================
    def create_stripe_payment_intent(self, wallet, username, usd_amount):
        """Create a Stripe payment intent for credit card payments"""
        try:
            import stripe
            stripe.api_key = STRIPE_SECRET_KEY
            
            mcx_amount = int(usd_amount / MCX_PRICE_USD)
            ...
            c = self.conn.cursor()
            c.execute("INSERT INTO stripe_payments (payment_id, wallet, username, usd_amount, mcx_amount, status, created_at, stripe_payment_intent) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                     (payment_id, wallet, username, usd_amount, mcx_amount, 'pending', time.time(), intent.id))
            self.conn.commit()
    ...
    def confirm_stripe_payment(self, payment_intent_id):
        """Confirm a Stripe payment and credit MCX"""
        c = self.conn.cursor()
        c.execute("SELECT payment_id, wallet, username, mcx_amount, status FROM stripe_payments WHERE stripe_payment_intent=?", (payment_intent_id,))
        row = c.fetchone()
        
        if not row:
            return {"success": False, "error": "Payment not found"}
        
        payment_id, wallet, username, mcx_amount, status = row
        
        if status == 'completed':
            return {"success": False, "error": "Payment already processed"}
        
        # Credit MCX to wallet
        self.balances[wallet] = self.balances.get(wallet, 0) + mcx_amount
        self._save_balance(wallet, self.balances[wallet])
        
        # Update payment status
        c.execute("UPDATE stripe_payments SET status='completed', completed_at=? WHERE payment_id=?", (time.time(), payment_id))
        self.conn.commit()
        ...
        # Record transaction
        tx_hash = hash_transaction({"from": "stripe", "to": wallet, "amount": mcx_amount})
        tx = Transaction(tx_hash, "stripe", wallet, mcx_amount, 0, time.time(), -1, "confirmed", "buy")
        self._save_transaction(tx)
        
        print(f"[STRIPE] ✅ Credited {mcx_amount} MCX to {wallet}")
        return {"success": True, "mcx_amount": mcx_amount, "wallet": wallet}
    
    def initiate_crypto_payment(self, wallet, username, usd_amount, method):
        """Initiate a crypto payment (BTC, ETH, USDC, DUCO)"""
        mcx_amount = int(usd_amount / MCX_PRICE_USD)
        
        if method == "btc":
            payment_address = BTC_PAYMENT_ADDRESS
        elif method == "eth":
            payment_address = ETH_PAYMENT_ADDRESS
        elif method == "usdc":
            payment_address = USDC_PAYMENT_ADDRESS
        elif method == "duco":
            payment_address = DUCO_PAYMENT_ADDRESS
        else:
            return {"success": False, "error": "Unsupported payment method"}
        ...
        c = self.conn.cursor()
        c.execute("INSERT INTO pending_crypto_payments (payment_id, wallet, username, method, amount, usd_amount, address, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                 (payment_id, wallet, username, method, mcx_amount, usd_amount, payment_address, 'pending', time.time()))
        self.conn.commit()
    ...
    def check_crypto_payment(self, payment_id, method, expected_amount):
        """Check if crypto payment has been received"""
        c = self.conn.cursor()
        ...
        if method == "btc":
            result = verify_btc_payment(BTC_PAYMENT_ADDRESS, expected_amount)
        elif method == "eth":
            result = verify_eth_payment(ETH_PAYMENT_ADDRESS, expected_amount)
        elif method == "usdc":
            result = verify_usdc_payment(USDC_PAYMENT_ADDRESS, expected_amount)
        elif method == "duco":
            result = verify_duco_payment(expected_amount)
        else:
    ...
    def get_payment_status(self, payment_id):
        ...
        c = self.conn.cursor()
        c.execute("SELECT status, mcx_amount, wallet FROM stripe_payments WHERE payment_id=?", (payment_id,))
        row = c.fetchone()
        if row:
            return {"status": row[0], "mcx_amount": row[1], "wallet": row[2]}
        
        # Check crypto payments
        c.execute("SELECT status, amount, wallet FROM pending_crypto_payments WHERE payment_id=?", (payment_id,))
        row = c.fetchone()
        if row:
            return {"status": row[0], "mcx_amount": row[1], "wallet": row[2]}
        ...
        try:
            async for message in websocket:
                ...
                # ========== BUY MCX WITH STRIPE ==========
                    ...
                    result = self.create_stripe_payment_intent(wallet, username, usd_amount)
                    await websocket.send(json.dumps({"type": "stripe_payment_intent", "data": result}))
                
                # ========== CONFIRM STRIPE PAYMENT ==========
                elif t == "confirm_stripe_payment":
                    payment_intent_id = data.get("payment_intent_id")
                    result = self.confirm_stripe_payment(payment_intent_id)
                    await websocket.send(json.dumps({"type": "stripe_payment_confirmed", "data": result}))
                
                # ========== BUY MCX WITH CRYPTO ==========
                elif t == "buy_mcx_crypto":
                    wallet = data.get("wallet")
                    username = data.get("username")
                    usd_amount = data.get("usd_amount", 0)
                    ...
                    result = self.initiate_crypto_payment(wallet, username, usd_amount, method)
                    await websocket.send(json.dumps({"type": "crypto_payment_instructions", "data": result}))
                
                # ========== CHECK CRYPTO PAYMENT ==========
                elif t == "check_crypto_payment":
                ...
                # ========== ADD LIQUIDITY ==========
                    ...
                    success, result = self.dex.add_liquidity(
...
# ==================== BLOCK PRODUCTION WITH REDISTRIBUTION ====================
    ...
    async def produce_block_for_level(self, original_level):
        ...
        if len(valid_sigs) >= MIN_VALIDATORS_PER_BLOCK:
            ...
            if original_level != effective_level:
                print(f"[BLOCK {block_id}] ✅ ACCEPTED | Original Level {original_level} → Produced by Level {effective_level} | Validators: {len(valid_sigs)}")
            else:
                print(f"[BLOCK {block_id}] ✅ ACCEPTED | Level {effective_level} | Validators: {len(valid_sigs)}")
            return True
        else:
            ...
            if original_level != effective_level:
                print(f"[BLOCK {block_id}] ❌ REJECTED | Original Level {original_level} → Attempted Level {effective_level} | Got {len(valid_sigs)}/{MIN_VALIDATORS_PER_BLOCK} signatures")
            else:
                print(f"[BLOCK {block_id}] ❌ REJECTED | Level {effective_level} | Got {len(valid_sigs)}/{MIN_VALIDATORS_PER_BLOCK} signatures")
            return False
    ...
    def distribute_block_reward(self, block, signers):
        ...
        if reward <= 0:
            print(f"[BLOCK {block.id}] No reward remaining for Level {block.level}")
            return
        ...
        print(f"[BLOCK {block.id}] REWARD: {reward} MCX | Level {block.level} | Validators: {validator_share} each")
    ...
    def slash_miner(self, vid, reason, block_id=-1):
        ...
        self.conn.commit()
        print(f"[SLASH] {m.username} lost {slash} MCX (now {m.stake} MCX, Level {m.level})")
        return slash
        ...
        for m in self.miners.values():
            if m.wallet == wallet:
                ...
                if self.level_mgr.has_permanent_tower(wallet):
                    print(f"[TOWER] {username} now has PERMANENT TOWER! Stake beyond Level 10")
...
class MicroCoreServer:
        ...
        while True:
            for challenge, pending in self.network.pending_challenges.items():
                ...
                if vid in pending["validators"] and vid not in pending["sigs"]:
                    ...
                    print(f"[EMBEDDED MINER] ✍️ Signed block {pending['bid']} (Level {pending['level']})")
    ...
    async def status_reporter_loop(self):
        while True:
            ...
            print(f"Total Minted: {total_minted:,} / {total_cap:,} ({percent:.4f}%)")
            ...
            print(f"Levels with Miners: {sorted(self.network.levels_with_miners)}")
            ...
            print(f"MCX Price: ${MCX_PRICE_USD:.4f} USD")
            ...
            for level in range(1, 11):
                ...
                print(f"Level {level}: {miner_count} miners | {cap - minted:,} / {cap:,} MCX remaining ({level_percent:.1f}%)")
    ...
    async def run(self):
        ...
        async with serve(self.network.ws_handler, NODE_HOST, NODE_PORT):
            ...
            print(f"{'='*60}")
            print(f"📊 LEVEL SYSTEM: 10 levels, 1,000 MCX per level")
            print(f"📊 BLOCK TIMES: L1:40s, L2:35s, L3:30s, L4:25s, L5:20s, L6:15s, L7:10s, L8:9s, L9:8s, L10:7s")
            print(f"📊 BLOCK REWARD: 3 MCX (halving per level)")
            print(f"📊 TOTAL CAP: ~546,840,000 MCX")
            print(f"📊 BLOCK REDISTRIBUTION: Inactive levels → level with most miners")
            print(f"📊 STRIPE INTEGRATION: Real credit card payments")
            print(f"📊 CRYPTO PAYMENTS: BTC, ETH, USDC, DUCO with blockchain verification")
            print(f"📊 BUYER REWARDS: Top 10 monthly (5000,3000,2000,1000,1000,500x5)")
            print(f"{'='*60}")
