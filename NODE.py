#!/usr/bin/env python3
"""
MICROCORE (MCX) COMPLETE NODE v19.0 - FINAL
...
# ==================== TOKENOMICS ====================
# Block reward is 3 MCX for ALL levels (base)
INITIAL_BLOCK_REWARD = 3
HALVING_INTERVAL = 4_204_800  # 4 years at average block time
...
# Level-specific block intervals (10 levels, separate pools)
...
# Level-specific hard caps (so all levels finish at same time)
LEVEL_CAPS = {
    1: 18_921_600,   # Level 1 total cap
    2: 21_624_000,   # Level 2 total cap
    3: 25_228_800,   # Level 3 total cap
    4: 30_274_560,   # Level 4 total cap
    5: 37_843_200,   # Level 5 total cap
    6: 50_457_600,   # Level 6 total cap
    7: 75_686_400,   # Level 7 total cap
    8: 84_096_000,   # Level 8 total cap
    9: 94_608_000,   # Level 9 total cap
    10: 108_123_360  # Level 10 total cap
}

# Reward Distribution Percentages (GLOBAL - from ALL blocks)
VALIDATOR_SHARE = 0.70      # 70% to validators
NODE_SHARE = 0.08           # 8% to nodes (global pool)
UPTIME_SHARE = 0.05         # 5% to uptime pool (global, split by miner uptime)
...
def sign_message_sha256(priv, msg):
    """SHA256 signing for web/Uno miners"""
...
class DEX:
    ...
    def get_supported_pools(self):
        ...
        return pools
# ==================== LEVEL MANAGER (TEMPORARY + PERMANENT TOWERS) ====================
class LevelManager:
    def __init__(self, net):
        self.net = net
        self.max_unlocked = 1
        self.temp_towers = {}  # Temporary towers (waiting for level unlock)
        self.perm_towers = {}  # Permanent towers (stake beyond level 10)
        self.level_wallets = {}
    
    def register(self, wallet, stake):
        """Register stake with temporary and permanent tower allocation"""
        alloc = {}
        rem = stake
        lvl = 1
        
        # First, allocate to unlocked levels (real stake)
        ...
        # Remaining becomes temporary towers (waiting for future levels)
        ...
        # Beyond MAX_LEVEL becomes permanent towers
        if rem > 0:
    ...
    def _update(self):
        ...
        while self.max_unlocked < MAX_LEVEL:
            ...
            if self.level_wallets.get(next_level, 0) >= MIN_WALLETS_FOR_NEXT_LEVEL:
                ...
                print(f"[LEVEL] Level {self.max_unlocked} UNLOCKED! (10 unique wallets at Level {self.max_unlocked-1})")
                self._convert_temporary_towers()
            else:
                break
    
    def _convert_temporary_towers(self):
        """Convert temporary towers to real stake when level unlocks"""
    ...
    def get_level(self, wallet):
        """Get miner's effective level (1-10)"""
    ...
    def has_permanent_tower(self, wallet):
        """Check if miner has permanent tower status (stake beyond level 10)"""
        return self.perm_towers.get(wallet, 0) > 0
    
    def get_permanent_tower_amount(self, wallet):
        """Get permanent tower stake amount"""
...
# ==================== MINER MANAGEMENT ====================
    def register_miner(self, vid, pub, username, wallet, stake, sig, ts, mtype):
        ...
        if lvl <= MAX_LEVEL:
            self.levels_with_miners.add(lvl)
        ...
        if self.level_mgr.has_permanent_tower(wallet):
            print(f"[TOWER] {username} has PERMANENT TOWER! +{self.level_mgr.get_permanent_tower_amount(wallet)} MCX beyond Level 10")
        return True
    ...
    def slash_miner(self, vid, reason, block_id=-1):
        ...
        if old_level != m.level:
            has_miners = False
            for miner in self.miners.values():
                if miner.active and miner.level == old_level and miner.vid != vid:
                    has_miners = True
                    break
            if not has_miners and old_level in self.levels_with_miners:
                self.levels_with_miners.discard(old_level)
            if m.level <= MAX_LEVEL:
                self.levels_with_miners.add(m.level)
        
        if m.slashes >= BAN_THRESHOLD:
            m.active = False
            if m.level in self.levels_with_miners:
                has_miners = False
                for miner in self.miners.values():
                    if miner.active and miner.level == m.level and miner.vid != vid:
                        has_miners = True
                        break
                if not has_miners:
                    self.levels_with_miners.discard(m.level)
    ...
    # ==================== REWARD DISTRIBUTION (GLOBAL POOLS) ====================
    def distribute_block_reward(self, block, signers):
        ...
        remaining = self.get_remaining_supply_for_level(block.level)
        percent = self.get_supply_percentage_for_level(block.level)
        print(f"[LEVEL {block.level} SUPPLY] {remaining:,} / {LEVEL_CAPS[block.level]:,} MCX remaining ({percent:.2f}%)")
...
# ==================== STAKING ====================
        ...
        for m in self.miners.values():
            if m.wallet == wallet:
                ...
                if old_level != m.level:
                    has_miners = False
                    for miner in self.miners.values():
                        if miner.active and miner.level == old_level and miner.vid != m.vid:
                            has_miners = True
                            break
                    if not has_miners and old_level in self.levels_with_miners:
                        self.levels_with_miners.discard(old_level)
    ...
    # ==================== LEVEL GROUPS & VALIDATOR SELECTION ====================
    ...
    def get_level_with_most_miners(self):
        """Find the level with the highest number of active miners (for block redistribution)"""
        best_level = 1
        best_count = 0
        for level in range(1, 11):
    ...
    def get_effective_production_level(self, original_level):
        """Determine which level actually produces the block (with redistribution)"""
...
# ==================== BLOCK PRODUCTION WITH REDISTRIBUTION ====================
    async def produce_block_for_level(self, original_level):
        """Produce a block - may be redirected to level with most miners if this level has insufficient miners"""
        
        # Determine actual production level (redistribution logic)
        effective_level = self.get_effective_production_level(original_level)
        
        if effective_level is None:
            return False
        
        # Select validators from the effective level
    ...
    async def produce_blocks_loop(self):
        """Main block production loop - rotates through all levels, redistributing inactive ones"""
        while True:
            self.update_level_groups()
            
            # Produce blocks for each level sequentially
            # Levels with no miners will have their blocks redistributed to the level with most miners
            for level in range(1, 11):
                success = await self.produce_block_for_level(level)
                if success:
                    # Wait for this level's block interval before next level's block
    ...
    def get_supply_percentage_for_level(self, level):
        c = self.conn.cursor()
        c.execute("SELECT minted, cap FROM level_supply WHERE level=?", (level,))
        row = c.fetchone()
        if row and row[1] > 0:
        ...
        try:
            async for message in websocket:
                ...
                if t == "register":
                    ...
                    success, result = self.dex.buy_mcx(
                        data["wallet"], data["usd_amount"], data.get("payment_method", "card")
                    )
                    await websocket.send(json.dumps({"type": "buy_result", "success": success, "data": result}))
